    ...
    public static class Acceleration
    {
        ...
        public static void Apply()
        {
            try
            {
                ...
                if (player == null) { MelonLogger.Warning("Acceleration: Player_Human not found."); return; }
                Vehicle vehicle = player.GetComponent<Vehicle>();
                if (vehicle == null) { MelonLogger.Warning("Acceleration: Vehicle not found."); return; }
                FieldInfo field = vehicle.GetType().GetField(StartupAccelerationField);
                if (object.ReferenceEquals(field, null)) { MelonLogger.Warning("Acceleration: field not found."); return; }
                if (originalStartupAcceleration < 0f)
                {
                    ...
                    { MelonLogger.Warning("Acceleration: value not float."); return; }
                }
