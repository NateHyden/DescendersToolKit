using MelonLoader;
using UnityEngine;
using System.Reflection;

namespace DescendersModMenu.Mods
{
    public static class FlyMode
    {
        public static bool Enabled { get; private set; } = false;

        public static float MoveSpeed = 30f;
        public static float ClimbSpeed = 20f;
        public static float LookSpeed = 90f; // degrees per second

        private static Vehicle _vehicle = null;
        private static Rigidbody _rb = null;
        private static Transform _playerTrans = null;
        private static VehicleController _vc = null;

        private static FieldInfo _physField = null; // bYxcVhv
        private static MethodInfo _toggleCtrl = null; // ToggleControl

        private static bool _savedKinematic = false;
        private static float _yaw = 0f;
        private static float _pitch = 0f;

        public static void Toggle()
        {
            Enabled = !Enabled;
            if (Enabled) Enable();
            else Disable();
            MelonLogger.Msg("[FlyMode] -> " + (Enabled ? "ON" : "OFF"));
        }

        private static void Enable()
        {
            try
            {
                GameObject player = GameObject.Find("Player_Human");
                if ((object)player == null) { MelonLogger.Warning("[FlyMode] Player_Human not found."); Enabled = false; return; }

                _playerTrans = player.transform;
                _vehicle = player.GetComponent<Vehicle>();
                _rb = player.GetComponent<Rigidbody>();
                _vc = player.GetComponent<VehicleController>();

                if ((object)_vehicle == null) { MelonLogger.Warning("[FlyMode] Vehicle not found."); Enabled = false; return; }

                if ((object)_physField == null)
                    _physField = typeof(Vehicle).GetField("bYxcVhv", BindingFlags.Public | BindingFlags.Instance);

                if ((object)_toggleCtrl == null)
                    _toggleCtrl = typeof(VehicleController).GetMethod("ToggleControl",
                        BindingFlags.Public | BindingFlags.Instance);

                // Make kinematic — we fully own transform now
                if ((object)_rb != null)
                {
                    _savedKinematic = _rb.isKinematic;
                    _rb.isKinematic = true;
                    _rb.velocity = Vector3.zero;
                    _rb.angularVelocity = Vector3.zero;
                }

                // Disable VehicleController input entirely
                if ((object)_vc != null && (object)_toggleCtrl != null)
                    _toggleCtrl.Invoke(_vc, new object[] { false, false });

                // Seed yaw/pitch from current rotation so we don't snap
                Vector3 euler = _playerTrans.eulerAngles;
                _yaw = euler.y;
                _pitch = 0f; // start level

                MelonLogger.Msg("[FlyMode] Ready. physField=" + ((object)_physField != null)
                    + " toggle=" + ((object)_toggleCtrl != null));
            }
            catch (System.Exception ex) { MelonLogger.Error("[FlyMode] Enable: " + ex.Message); Enabled = false; }
        }

        private static void Disable()
        {
            try
            {
                if ((object)_vehicle != null && (object)_physField != null)
                    _physField.SetValue(_vehicle, true);

                if ((object)_rb != null)
                {
                    _rb.isKinematic = _savedKinematic;
                    _rb.velocity = Vector3.zero;
                    _rb.angularVelocity = Vector3.zero;
                }

                // Re-enable input
                if ((object)_vc != null && (object)_toggleCtrl != null)
                    _toggleCtrl.Invoke(_vc, new object[] { true, true });
            }
            catch (System.Exception ex) { MelonLogger.Error("[FlyMode] Disable: " + ex.Message); }

            _vehicle = null;
            _rb = null;
            _playerTrans = null;
            _vc = null;
        }

        public static void Tick()
        {
            if (!Enabled) return;
            if ((object)_vehicle == null || (object)_playerTrans == null) return;

            try
            {
                // Kill vehicle physics every frame
                if ((object)_physField != null)
                    _physField.SetValue(_vehicle, false);

                InControl.InputDevice dev = InControl.InputManager.ActiveDevice;

                // ── Right stick — update yaw/pitch ────────────────────────
                float rsX = (float)dev.RightStick.X;
                float rsY = (float)dev.RightStick.Y;

                if (Mathf.Abs(rsX) > 0.1f)
                    _yaw += rsX * LookSpeed * Time.deltaTime;
                if (Mathf.Abs(rsY) > 0.1f)
                    _pitch -= rsY * LookSpeed * Time.deltaTime; // - so stick up = look up
                _pitch = Mathf.Clamp(_pitch, -89f, 89f);

                // Set bike rotation
                Quaternion newRot = Quaternion.Euler(_pitch, _yaw, 0f);
                _playerTrans.rotation = newRot;

                // BikeCamera ignores oSVdqtx when airborne — force camera directly
                Camera cam = Camera.main;
                if ((object)cam != null)
                {
                    // Position camera behind and above the bike
                    Vector3 camOffset = new Vector3(0f, 1.5f, -4.5f);
                    cam.transform.position = _playerTrans.position + newRot * camOffset;
                    cam.transform.rotation = newRot;
                }

                // ── Left stick — move relative to where we're looking ─────
                float v = (float)dev.LeftStick.Y;
                float h = (float)dev.LeftStick.X;

                _playerTrans.position += newRot * Vector3.forward * v * MoveSpeed * Time.deltaTime;
                _playerTrans.position += newRot * Vector3.right * h * MoveSpeed * Time.deltaTime;

                // ── Triggers — up/down ────────────────────────────────────
                float up = (float)dev.RightTrigger;
                float down = (float)dev.LeftTrigger;
                if (Input.GetKey(KeyCode.Space)) up = 1f;
                if (Input.GetKey(KeyCode.LeftShift)) down = 1f;
                _playerTrans.position += Vector3.up * (up - down) * ClimbSpeed * Time.deltaTime;
            }
            catch (System.Exception ex)
            {
                MelonLogger.Error("[FlyMode] Tick: " + ex.Message);
                Enabled = false;
                Disable();
            }
        }

        public static void Reset()
        {
            if (Enabled) { Enabled = false; Disable(); }
            _physField = null;
            _toggleCtrl = null;
        }
    }
}